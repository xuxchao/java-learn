...
1. RabbitMQ：需要记住：连接器 Connection, 通道 channel, 交换机 exchange, 队列 queue, 生产者 producer， 消费者 consumer 七个概念。然后 交换机有 direct, faount, header, topic 四种模式。同时如何防丢：生产者需要开启确认模式，消费者确认重新入队，死信的处理。不重复：业务侧做幂等，有序保障：一个消费者的时候是有序的，多个消费者速度不一样可能是无序，另外失败重拍也是无序。
MQ 的核心作用是消峰解耦。数据库的并发可能是 100，MQ 的并发是 1000。可以数据存入 MQ 慢慢给数据库下发消息。解耦的例子，登录了可能涉及初始积分、短信通知、站内信通知等等多个业务模块。登录只需要生产一下，其他业务跟登录不会有直接关联，通过 MQ 进行处理