...
> 状态：已综合会话决策 + 优化体检（v2）。待发布至项目 issue tracker（需先配置 tracker 并运行 `/setup-matt-pocock-skills` 获得 `ready-for-agent` 标签词汇）。
...
- 发布说明：本 spec 已发布至本地 issue tracker（`.scratch/java-backend-plan/spec.md`），标签 `ready-for-agent`。tracker 配置见 `docs/agents/issue-tracker.md`。
